...
DROP function IF EXISTS `educdb_test`.`URLENCODE`;
;
...
DROP function IF EXISTS `educdb_test`.`getCoursId`;

DELIMITER $$
USE `educdb_test`$$
CREATE FUNCTION `educdb_test`.`getCoursId`( _nomCours varchar(45))
RETURNS int
DETERMINISTIC
BEGIN
...
DROP function IF EXISTS `educdb_test`.`getChapId`;

DELIMITER $$
USE `educdb_test`$$
CREATE FUNCTION `educdb_test`.`getChapId`(_nomCours varchar(45), _titreChapitre varchar(45))
RETURNS int
DETERMINISTIC
BEGIN
   DECLARE _idCours int;
...
DROP function IF EXISTS `educdb_test`.`getQuizId`;

DELIMITER $$
USE `educdb_test`$$
CREATE FUNCTION `educdb_test`.`getQuizId`(_nomQuiz varchar(45), _nomChapitre varchar(45), _nomCours varchar(45))
RETURNS int
DETERMINISTIC
BEGIN
   DECLARE _idQuiz int ;
...
DROP function IF EXISTS `educdb_test`.`getQuestionId`;

DELIMITER $$
USE `educdb_test`$$
CREATE FUNCTION `educdb_test`.`getQuestionId`( _nomQuestion varchar(45))
RETURNS int
DETERMINISTIC
BEGIN
